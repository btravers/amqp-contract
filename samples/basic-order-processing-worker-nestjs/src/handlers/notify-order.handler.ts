...
export class NotifyOrderHandler {
  ...
  async handle(message: WorkerInferConsumerInput<typeof orderContract, "notifyOrder">): Promise<void> {
  ...
  handler = defineHandler(orderContract, "notifyOrder", (message) => this.handle(message));
}
