...
export class HandleUrgentOrderHandler {
  ...
  handleMessage = async (message: WorkerInferConsumerInput<typeof orderContract, "handleUrgentOrder">) => {
    this.logger.warn(
  ...
  handler = defineHandler(orderContract, "handleUrgentOrder", async (message) => this.handleMessage(message));
}
