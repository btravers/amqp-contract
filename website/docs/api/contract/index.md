...
```ts
type InferConsumerNames<TContract> =
...
```ts
type InferPublisherNames<TContract> =
...
```ts
function defineConsumer<TMessage>(queue, message, options?): ConsumerDefinition<TMessage>;
```
...
| `THeaders` _extends_ \| `undefined` \| `StandardSchemaV1`\<`Record`\<`string`, `unknown`\>, `Record`\<`string`, `unknown`\>\> | `undefined`  |
