...
- **Image**: `rabbitmq:4.2.1-management-alpine` (default)
  ...
  - In CI environments, uses `ghcr.io/btravers/amqp-contract/rabbitmq:latest` to avoid Docker Hub rate limits and enable faster image pulls from the same registry
- **Ports**:
  - 5672 (AMQP)
...
Check out the [examples directory](https://github.com/btravers/amqp-contract/tree/main/samples) in the repository for more testing examples.
