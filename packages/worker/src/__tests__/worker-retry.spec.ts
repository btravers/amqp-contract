...
describe("Worker Retry Mechanism", () => {
  ...
  describe("Quorum Native Retry Mode", () => {
    it("should use RabbitMQ's native delivery limit for retry handling", async ({
      ...
      // No wait queue is created in quorum-native mode
      await vi.waitFor(
        () => {
          if (attemptCount < 2) {
            throw new Error("Message not yet processed twice");
      ...
      // Verify no wait queue was created
      try {
        await amqpChannel.checkQueue("quorum-native-queue-wait");
        // If we get here, the queue exists which is unexpected
        throw new Error("Wait queue should not exist in quorum-native mode");
      } catch (error) {
    ...
    it("should send message to DLQ after exceeding deliveryLimit", async ({
      ...
      const queue = defineQueue("quorum-dlq-queue", {
        type: "quorum",
        deliveryLimit: 2, // After 2 delivery attempts, dead-letter
        deadLetter: {
          exchange: dlx,
      ...
      // THEN message should be dead-lettered after 2 delivery attempts
      // Note: x-delivery-count starts at 0 for first delivery, so deliveryLimit of 2
      // means message is dead-lettered when x-delivery-count reaches 2 (i.e., after 2 requeues)
      await vi.waitFor(
        async () => {
      ...
      // Message should have been processed deliveryLimit times (2 attempts)
